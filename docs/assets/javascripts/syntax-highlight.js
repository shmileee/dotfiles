(() => {
  ...
  const commandWrappers = new Set(["command", "env", "exec", "nohup", "sudo", "time"]);
  ...
  function highlightCommand(code) {
    ...
    while (index < source.length) {
      ...
      if (character === "$") {
        const variable = source.slice(index).match(/^\$(?:\{[^}\n]+\}|[A-Za-z_][A-Za-z0-9_]*|[@*#?$!-])/);
        if (variable) {
      ...
      const option = source.slice(index).match(/^(?:--|--?[A-Za-z0-9][A-Za-z0-9_-]*(?:=[^\s|;&<>]+)?)/);
      if (option) {
      ...
      if (expectCommand) {
        ...
        type = "keyword";
        subcommandsLeft -= 1;
      } else if (word === "." || word === ".." || /^(?:\.?\.?\/|\/|~\/|https?:\/\/)/.test(word) || word.includes("/")) {
        type = "path";
  ...
  function setupSyntaxHighlighting() {
    document.querySelectorAll(".md-typeset .language-bash code").forEach(highlightCommand);
  }

  if (typeof document$ !== "undefined") {
    document$.subscribe(setupSyntaxHighlighting);
  } else if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", setupSyntaxHighlighting, { once: true });
  } else {
